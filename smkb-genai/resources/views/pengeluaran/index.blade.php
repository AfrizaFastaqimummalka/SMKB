...
@section('content')
    <div class="flex items-center justify-between mb-5 gap-3 flex-wrap">
        ...
        <a href="{{ route('pengeluaran.create') }}" class="rounded-lg bg-navy-950 text-white text-sm font-medium px-4 py-2 hover:bg-navy-800 whitespace-nowrap">
            + Catat Pengeluaran
        </a>
    </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($pengeluaran as $p)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <span class="text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-600">{{ str($p->kategori)->headline() }}</span>
                        </td>
                        <td class="px-4 py-3 text-right font-medium text-red-700">Rp {{ number_format($p->jumlah, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right">
                            <form method="POST" action="{{ route('pengeluaran.destroy', $p) }}" onsubmit="return confirm('Hapus pengeluaran ini?')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-slate-400">Belum ada data pengeluaran.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
