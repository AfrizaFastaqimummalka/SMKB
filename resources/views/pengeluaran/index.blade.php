...
@section('subtitle', count($pengeluaran) . ' transaksi — Total: Rp ' . number_format($pengeluaran->sum('jumlah'), 0, ',', '.'))
...
@section('content')
        ...
        <table>
            ...
            <tbody>
                @forelse ($pengeluaran as $i => $p)
                    <tr>
                        <td style="color:#9ca3af;">{{ $pengeluaran->firstItem() + $i }}</td>
                        <td>{{ $p->tanggal->format('d/m/Y') }}</td>
                        <td>
                            <span class="badge badge-red">{{ str($p->kategori)->headline() }}</span>
                        </td>
    ...
    @if ($pengeluaran->hasPages())
        <div style="margin-top:16px;">{{ $pengeluaran->links() }}</div>
    @endif
